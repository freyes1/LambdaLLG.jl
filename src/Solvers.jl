...
`DiscreteCallback`. The optional 1D Zhang-Li spin-transfer torque is included
when `p.stt_active` is `true`, using the `u_stt` and `beta_stt` fields stored
in `p`. The STT discretization assumes lattice-spacing units with `a = 1`.
...
Spin normalization is enforced at each accepted step via a
`DiscreteCallback`. The optional 2D Zhang-Li spin-transfer torque is included
when `p.stt_active` is `true`, using the `u_stt` and `beta_stt` fields stored
in `p`. The STT discretization assumes lattice-spacing units with `a = 1`.
Returns the `ODESolution`.
"""

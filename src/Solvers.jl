function evolve1D(s0, tspan, p::LLGParams1D; reltol = 1e-6, abstol = 1e-6)
    # A callback function could be used to include time dependent parameters
    cb = DE.DiscreteCallback((u,t,integrator)->true,
                      integrator->normalize_spins1D!(integrator.u, integrator.p, integrator.t))
...
function evolve2D(s0, tspan, p::LLGParams2D; reltol = 1e-6, abstol = 1e-6)
    cb = DE.DiscreteCallback((u,t,integrator)->true,
                      integrator->normalize_spins2D!(integrator.u, integrator.p, integrator.t))
